...
async function parseRecipe(url) {
  ...
  const prompt = createLLMPrompt(extractedIngredients);
...
function createLLMPrompt(ingredients) {
  const prompt = `You are a recipe ingredient parser. Extract and standardize ingredients from the following list. Return ONLY a valid JSON object with this exact structure:
